...
- **Clean Mesh** : チェックを入れることでMESHデータをクリンナップし軽量化します。  
- **Select Dirctory and Convert** : ディレクトリを選択し、変更を実行します。  
...
### 3-7.  Node Inspector: ノード情報を読み込みと書き出し  
...
```  
### 3-9.  Node Inspector: 質量とイナーシャの設定  
...
### 3-10.  Node Inspector: 回転軸と可動範囲の設定  
...
### 3-11.  Node Inspector: アクチュエータのパラメータ設定  
...
### 3-14.  URDFもしくはMJCFの出力  
  
ロボットモデルの出力を行います。  
  
- **Export URDF** : URDFファイルを出力します。ディレクトリを生成し、その中にurdfファイルとMESHを保存します。  
- **Export for Unity** : UnityのURDF-importで扱える形式で出力します。  
