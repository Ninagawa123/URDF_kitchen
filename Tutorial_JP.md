...
書き出し時の**単位はメートル**を指定してください。(デフォルト設定がmmの場合があるのでご注意)  
...
- **Scaler** : MESHを1000倍、0.001にします。  
- **Rotation** : MESH自体を回転します。  
...
設定したパラメータはMESH名.xmlというファイルとして保存されます。  
...
```  
例）Import Meshボタンで、c_chest.stlを読み込む。  
　　Tボタンを押すと3DビューのMESHがワイヤーフレームとなり、重心を示す赤い球が確認できる。  
```  
### 2-3. 質量とイナーシャの設定  
...
子ユニットを接続する座標を設定します。座標は複数設定することができます。例えば、腰パーツであれば、右股関節、左股関節、上半身を接続するポイントを設定するなどです。  
...
複数のポイントにチェックを入れてマーカーを同時に動かしたり、左右対称の場合はY:の値をコピペして正負反転させるなどができます。  
...
- **Batch Mirror "l_" to "r_" Meshes and XMLs** : 指定ディレクトリ内のユニットについて一括でミラーのMESHとXMLを作成します。  
  
この工程でも、**左半身および中央のユニット**がすべて出力できたら作業完了です。  
最後に**Batch Mirror "l_" to "r_" Meshes and XMLs** をすることにより、ミラーの右半身をまとめて出力することができます。  
...
```  
例）Batch Mirror "l_" to "r_" Meshes and XMLsを押し、  
...
カラーパレットのCustom colors欄のボックスをクリックしてから色をピックし、Add to Custom colorsボタンを押すことで、カラーを保存できます。複数のパーツに同じ色を割り当てる際に便利です。  
...
作業の状況は保存することができます。  
- Save Project : 作業を保存します。ファイル名にタイムスタンプが付きます。  
...
接続が完了したら作業ができたら一度プロジェクトをセーブします。  
...
- **Export for Unity** : UnityのURDF-importで使える形式で出力します。  
