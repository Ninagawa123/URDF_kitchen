...
class FooNode(BaseNode):
    ...
    def show_inspector(self):
        print(f"Showing inspector for node: {self.name()}")
...
class InspectorWindow(QtWidgets.QWidget):
    ...
    def on_axis_selection_changed(self, button):
        ...
        if self.current_node:
            ...
            print(f"Rotation axis changed to: {['X', 'Y', 'Z'][axis_id]}")
...
class STLViewerWidget(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.renderer.SetAmbient(0.8, 0.8, 0.8)
    ...
    def reset_camera(self):
        """カメラビューをリセットし、すべてのアクターをビューに収める"""
        if not self.renderer.GetActors().GetNumberOfItems():
            self.setup_camera()
            return

        # すべてのアクターのバウンディングボックスを計算
        ...
        # カメラの位置を設定
        distance = diagonal * 1.5  # バウンディングボックスの1.5倍の距離
    ...
    def load_stl(self, file_name=None):
        """STLファイルを読み込む"""
        if not file_name:
            file_name, _ = QtWidgets.QFileDialog.getOpenFileName(
                self, "Open STL File", "", "STL Files (*.stl)")
...
class CustomNodeGraph(NodeGraph):
    ...
    def export_urdf(self):
        ...
        try:
            ...
            # LoadProjectもしくはSaveProjectのパスからの検索
            if self.last_save_dir or self.project_dir:
                base_dir = self.last_save_dir or self.project_dir
                # 2階層上に移動
                parent_dir = os.path.dirname(os.path.dirname(base_dir))
                
                # urdfもしくはURDFフォルダを探す
                for dir_name in ['urdf', 'URDF']:
                    possible_urdf_dir = os.path.join(parent_dir, dir_name)
                    if os.path.exists(possible_urdf_dir) and os.path.isdir(possible_urdf_dir):
                        default_dir = possible_urdf_dir
                        break
                
                # URDFフォルダが見つからない場合はmeshesと同じ階層を使用
                if not default_dir and self.meshes_dir:
                    default_dir = os.path.dirname(self.meshes_dir)

            # LoadProject/SaveProjectを使用していない場合
            if not default_dir and self.meshes_dir:
                default_dir = os.path.dirname(self.meshes_dir)

            # デフォルトディレクトリが設定されていない場合、現在のディレクトリを使用
            if not default_dir:
                default_dir = os.getcwd()

            # 保存先の選択
            urdf_file, _ = QtWidgets.QFileDialog.getSaveFileName(
                self.widget,
                "Export URDF",
                os.path.join(default_dir, f"{self.robot_name}.urdf"),
                "URDF Files (*.urdf)"
            )
            ...
            with open(urdf_file, 'w', encoding='utf-8') as f:
                ...
                if base_node:
                    self._write_urdf_node(f, base_node, None, visited_nodes, materials)
            ...
            print(f"Error exporting URDF: {str(e)}")
            traceback.print_exc()
            return False
    ...
    def _write_urdf_node(self, file, node, parent_node, visited_nodes, materials):
        ...
        if not is_decoration:
            ...
            if node.name() == "base_link":
                ...
                # ジョイントの書き出し
                if parent_node:
                    # base_linkからのジョイントの特別処理
                    if parent_node.name() == "base_link":
                        joint_name = f"base_to_{node.name()}"
                        ...
                        file.write('    <limit effort="0.0" lower="0.0" upper="0.0" velocity="0.0"/>\n')
                        file.write('  </joint>\n\n')
                    else:
                        # 通常のジョイント処理
                        ...
                        for port in parent_node.output_ports():
                            for connected_port in port.connected_ports():
                                if connected_port.node() == node:
                                    port_idx = int(port.name().split('_')[1]) - 1
    ...
    def show_inspector(self, node):
        ...
        try:
            ...
            # ノードの属性を初期化（必要な場合）
            if not hasattr(node, 'node_color'):
                node.node_color = [1.0, 1.0, 1.0]
            if not hasattr(node, 'rotation_axis'):
                node.rotation_axis = 0
            ...
            if self.widget and self.widget.window():
                ...
                inspector_x = main_geo.x() + main_geo.width() + 10
                inspector_y = main_geo.y()
                self.inspector_window.move(inspector_x, inspector_y)
            ...
            print(f"Showing inspector for node: {node.name()}")
    ...
    def save_project(self, file_path=None):
        ...
        try:
            ...
            ET.SubElement(root, "robot_name").text = self.robot_name
    ...
    def import_xmls_from_folder(self):
        ...
        folder_path = QtWidgets.QFileDialog.getExistingDirectory(
            None, "Select Folder Containing XML Files")
