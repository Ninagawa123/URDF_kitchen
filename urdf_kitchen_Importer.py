...
class MJCFParser:
    ...
    def _parse_with_includes(self, mjcf_file_path, working_dir=None):
        ...
        try:
            # Parse main file. UTF-8 で失敗した場合は、Windows の platform default
            # (cp932) 等で保存された MJCF を救済するため cp932/latin-1 で decode し
            # 直して parse を再試行する。Assembler の Export MJCF 側 (v202608 以前) は
            # encoding 指定なしで書き出しており、Windows-JP 環境で日本語コメントを
            # 含むファイルが cp932 で保存され、UTF-8 として parse すると 0x81 等の
            # バイトで ParseError を起こす。
            try:
                tree = ET.parse(mjcf_file_path)
                root = tree.getroot()
            except ET.ParseError as _pe:
                with open(mjcf_file_path, 'rb') as _bf:
                    raw = _bf.read()
                _last_error = _pe
                _rescued = False
                for _enc in ('cp932', 'shift_jis', 'euc_jp', 'latin-1'):
                    try:
                        _text = raw.decode(_enc)
                    except Exception:
                        continue
                    # XML declaration 中の encoding 指定と実バイトが食い違わないよう
                    # declaration があれば剥がしてから fromstring に渡す。
                    if _text.lstrip().startswith('<?xml'):
                        _idx = _text.find('?>')
                        if _idx != -1:
                            _text = _text[_idx + 2:]
                    try:
                        ...
                        if self.verbose:
                            print(f"MJCF file was not valid UTF-8; recovered by "
                                  f"decoding as {_enc}: {mjcf_file_path}")
                        _rescued = True
                        break
                    except ET.ParseError as _pe2:
                        _last_error = _pe2
                        continue
                if not _rescued:
                    raise _last_error
...
# MJCF/URDF 出力を UTF-8 で統一するための共通ヘルパ。
# Windows-JP など platform default codec が cp932 の環境で open() を
# encoding 無しで使うと、日本語コメント等が cp932 で書き出され、
# ET.parse (UTF-8 前提) で「not well-formed (invalid token)」に化ける
# ため、書き込みは必ずこのヘルパを経由する。
MJCF_XML_DECLARATION = '<?xml version="1.0" encoding="UTF-8"?>\n'


def open_mjcf_for_write(file_path):
    """Open an MJCF (or MJCF-style XML) file for writing with forced UTF-8 encoding.
